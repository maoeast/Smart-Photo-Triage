...
<body data-page="settings">
  ...
  <main>
    <section class="panel ai-panel" aria-labelledby="ai-title">
      ...
      <div class="privacy-strip"><span class="privacy-strip__icon">◎</span><p><strong>隐私提示：</strong>默认不会联网。启用互联网模型前需要您明确确认；访问密钥只从本机环境变量读取，不会写入工作区、配置或日志。</p></div>
      <div class="ai-grid">
        <div class="subpanel">
          ...
          <label class="field"><span>访问密钥变量名（高级设置）</span><input id="api_key_env" placeholder="例如 OPENAI_API_KEY，不填写实际密钥"><small>请先在启动界面的 PowerShell 中设置密钥。这里仅记录变量名称，绝不保存密钥本身。</small></label>
          <button class="button button--primary" type="button" data-action="save-provider">保存分析服务</button>
        </div>
        <div class="subpanel registry-panel">
          ...
          <p class="hint">“访问密钥已准备”仅表示相应环境变量存在，不会显示、保存或传输密钥内容。</p>
        </div>
      </div>
...
function payload(){ const selectedService=choice("driver"), inferredName=selectedService==="deepseek"&&!text("display_name")?"DeepSeek":text("display_name"); return {source:text("source"),output:text("output"),plan_id:text("plan_id"),transaction_id:text("transaction_id"),confirmation:"",provider_id:text("provider_id"),display_name:inferredName,driver:selectedService==="deepseek"?"openai_compatible":selectedService,model:text("model"),base_url:text("base_url"),api_key_env:text("api_key_env"),item_primary:choice("item_primary"),item_fallbacks:text("item_fallbacks"),item_confidence_below:text("item_confidence_below"),item_escalate_to:choice("item_escalate_to"),burst_primary:choice("burst_primary"),burst_fallbacks:text("burst_fallbacks"),burst_confidence_below:text("burst_confidence_below"),burst_escalate_to:choice("burst_escalate_to"),allow_cloud:checked("allow_cloud"),allow_lan:checked("allow_lan"),cloud_confirmation:text("cloud_confirmation"),lan_confirmation:text("lan_confirmation")}; }
async function api(action, body){ const response=await fetch("/api/"+action,{method:"POST",headers:{"Content-Type":"application/json","X-SPT-CSRF":csrf},body:JSON.stringify(body)}); const data=await response.json(); if(!response.ok)throw new Error(data.detail || data.error || "LOCAL_OPERATION_FAILED"); return data; }
async function call(action){ try{ const body=payload(); if(action==="execute"){const approved=window.confirm("即将根据已批准的计划创建照片副本。原照片不会移动或删除。是否继续真实复制？");if(!approved){status("已取消真实复制，未创建任何文件副本。");return;}body.confirmation="EXECUTE";}if(action==="rollback"){const approved=window.confirm("即将撤回本次真实复制创建的文件副本。原照片不会受到影响。是否继续安全回滚？");if(!approved){status("已取消安全回滚，现有副本保持不变。");return;}body.confirmation="ROLLBACK";}status("正在执行本地操作…");const data=await api(action,body);status(JSON.stringify(data,null,2));if(data.plan_id)setText("plan_id",data.plan_id);if(data.applied&&data.applied.transaction_id)setText("transaction_id",data.applied.transaction_id);if(data.ai)applyBootstrap({...data.ai,workspace:byId("workspace")?.value || ""});if(data.url)window.open(data.url,"_blank","noopener");}catch(error){status("操作未完成："+error.message);} }
async function choose(field){ try{ status("正在打开本机目录选择器…"); const data=await api("choose-directory",{field}); if(data.selected)setText(field,data.selected); if(field==="workspace"){setText("workspace",data.workspace);if(data.ai)applyBootstrap({...data.ai,workspace:data.workspace});}if(!data.selected)status("未选择目录，当前内容保持不变。");}catch(error){status("无法打开目录选择器："+error.message);} }
